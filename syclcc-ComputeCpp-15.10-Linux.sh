...
#  $HOST_CXX -std=c++0x -pthread $OBJFILES -o $OFILE -rdynamic $OPTS -O2 $DEBUG $CFLAGS $LIBPATHS -L$COMPUTECPP/lib/ -lSYCL -lOpenCL $LDFLAGS
  $HOST_CXX -std=c++0x -pthread $OBJFILES -o $OFILE -rdynamic $OPTS -O2 $DEBUG $CFLAGS $LIBPATHS -L$COMPUTECPP/lib/ -Wl,-rpath=$COMPUTECPP/lib -lSYCL -lOpenCL $LDFLAGS
fi
